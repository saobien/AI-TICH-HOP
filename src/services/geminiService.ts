...
export async function integrateAIIntoLessonPlan(apiKey: string, data: LessonPlanData, model: string = "gemini-3-flash-preview") {
  ...
  // Extract HTML if Gemini wraps it in markdown code blocks
  let text = response.text || '';
  const htmlMatch = text.match(/<html[\s\S]*<\/html>/i) || text.match(/<body[\s\S]*<\/body>/i) || [text];
  
  return htmlMatch[0].replace(/```html|```/g, '').trim();
}
